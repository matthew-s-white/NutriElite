...
async function getFriendWeights(emailOrUsername) {
    try {
        const weights = await client.collection("weightrecords").getFullList({
            filter: `user.email = "${emailOrUsername}" || user.username = "${emailOrUsername}"`,
            sort: "+created"
